...
Type: Action
Purpose:
Sends a confirmation email to the client ("Form Received + What's Next") assuring them that their data was successfully submitted and setting expectations for the next steps (e.g., the delivery of their AI Audit Shopping List).
...
This GoHighLevel workflow acts as the command center for the post-purchase audit phase. Triggered by the submission of the Deep Onboarding Form, it immediately updates the client's CRM status, tags the contact, and sends them professional email/SMS confirmations. Crucially, it handles the data handoff by firing a webhook to n8n, which initiates the multi-agent AI pipeline to build the actual Automation Shopping List, while simultaneously alerting the Selr AI team to prepare for the upcoming discovery call.
